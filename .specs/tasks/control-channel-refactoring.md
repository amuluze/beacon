...
> 当前状态：阶段一反向隧道、基础 Agent 生命周期和 `/api/v1/agent/list` 已落地；本文保留为后续终端、录制、审计和资产能力的规划背景。
...
控制通道历史上基于 rpcx RPC；当前实现已经切换为 Agent 主动连接 Server 的反向 gRPC tunnel，通过 `common/rpc/tunnel` 和 `amprobe/pkg/rpc.Caller` 按 Agent 标识路由到目标 Agent。

### 1.2 通信拓扑

Agent 主动连接 Server 控制端口，Server 维护 Agent 生命周期状态，前端通过 Agent 列表 API 获取可选节点。
...
P0 阶段二：动态注册 + 心跳（部分落地）
   └── 已有 Agent 生命周期和列表；Join Token 注册、吊销和更完整详情仍需补齐
