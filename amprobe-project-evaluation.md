...
| **Go 代码量** | ~14,970 行（amprobe 9,822 / collia 2,670 / common 2,478） |
| **Go 文件数** | 173 个 |
| **前端文件数** | 196+ 个（Vue + TypeScript） |
| **Go 模块** | 3 个（amprobe / collia / common） |
| **前端项目** | 2 个（amprobe-web + website） |
...
- **依赖方向正确**：`common` 作为无内部依赖的共享契约层，被 `amprobe` 和 `collia` 共同导入。
- **技术选型清晰**：
...
| `amprobe/service/report` | `report_test.go` | 123 行 | 缺失 Agent 拒绝、完整批次持久化 | ✅ 核心路径已覆盖 |
| `amprobe/service/task` | `task_test.go` | 77 行 | 告警任务多 Agent 独立评估 | ✅ 场景正确 |
| `amprobe/service/host/repo` | `host_test.go` | 94 行 | 网络使用率 DB 错误不降级 | ⚠️ 范围有限 |
| `amprobe/service/container/repo` | `container_test.go` | 101 行 | 容器 RPC 调用 | ⚠️ 范围有限 |
| `amprobe/pkg/fiberx` | `fiberx_test.go` | 43 行 | 响应辅助函数 | ✅ 正确 |
| `amprobe/pkg/errors` | `errors_test.go` | 25 行 | 错误分类 | ✅ 正确 |
...
| 配置安全 | ⚠️ 有隐患 | 开发配置 `SigningKey = "amprobe"` 为硬编码弱密钥，配置中显式警告了默认值风险 |
...
| 风险 | 详情 | 建议 |
|------|------|------|
| **Go 版本漂移** | `common` 用 `1.25.0`，`amprobe`/`collia` 用 `1.21.10` | 统一至同一 Go 版本，降低兼容性风险 |
...
| 风险 | 位置 | 严重度 | 建议 |
|------|------|--------|------|
| 硬编码 JWT 签名密钥 | `configs/config.toml:46` 和 `config.dev.toml:46` | 🔴 高 | 生产部署必须替换为强随机密钥，当前已输出 `slog.Warn` 警告但开发配置仍为 `"amprobe"` |
...
3. **统一 Go 版本**：将 `amprobe` 和 `collia` 从 `1.21.10` 升级至 `1.25.0`，与 `common` 保持一致。
...
**Amprobe 是一个架构设计合理、文档治理优秀的 Server-Agent 监控平台项目。** 其核心亮点包括：
