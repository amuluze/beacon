...
func ReportInstallation(ctx context.Context, config *Config) {
	...
	req.Header.Set("User-Agent", "amprobe-install-reporter")
...
func buildInstallReportPayload(config *Config) (installReportPayload, error) {
	...
	return installReportPayload{
		InstallID:     installID,
		Image:         os.Getenv("AMPROBE_IMAGE"),
		Version:       os.Getenv("AMPROBE_VERSION"),
		PublicBaseURL: firstNonEmpty(os.Getenv("AMPROBE_PUBLIC_BASE_URL"), config.AgentInstall.PublicBaseURL),
		InstallDir:    config.InstallReport.InstallDir,
		HTTPPort:      firstNonEmpty(os.Getenv("AMPROBE_HTTP_PORT"), intString(config.Fiber.Port)),
		ControlPort:   firstNonEmpty(os.Getenv("AMPROBE_CONTROL_PORT"), intString(config.AgentInstall.ControlPort)),
		ContainerName: os.Getenv("AMPROBE_CONTAINER_NAME"),
		Hostname:      hostname,
	}, nil
}
